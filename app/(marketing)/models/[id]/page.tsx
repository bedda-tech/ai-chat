...
function getTierInfo(modelId: string): {
  ...
  return {
    label: "Plus+",
    plan: "Plus, Pro, or Max plan",
    price: "from $12/mo",
  };
}

function getModelFaq(
  model: Model,
  isFree: boolean
): { q: string; a: string }[] {
  ...
  return [
    ...
    },
    {
      ...
      a: `${model.name} supports a ${formatContextWindow(model.contextWindow)} context window${model.contextWindow >= 200_000 ? ", which means it can process entire codebases, lengthy documents, and long conversation histories in a single prompt" : model.contextWindow >= 32_000 ? ", suitable for most documents and long coding tasks" : ", optimized for fast responses on shorter tasks"}.`,
    },
    {
    ...
    <div className="min-h-screen bg-background">
      ...
      <section className="border-b bg-gradient-to-b from-muted/30 to-background px-4 py-16 text-center">
        <div className="mx-auto max-w-3xl">
          <div className="mb-4 flex items-center justify-center gap-2">
            <span
              className={`rounded-full px-3 py-1 font-medium text-sm ${providerColor}`}
            >
              {providerLabel}
            </span>
            <span
          ...
          {isFree && (
            <p className="mt-3 text-muted-foreground text-sm">
              Free forever · 500 messages/month
            </p>
          )}
        </div>
      </section>
      ...
      <section className="border-y bg-muted/30 px-4 py-14">
        <div className="mx-auto max-w-2xl text-center">
          <h2 className="mb-4 font-bold text-2xl">
            {isFree
              ? `${model.name} is free on bedda.ai`
              : `Access ${model.name} for $12/mo`}
          </h2>
          ...
          <div className="mx-auto mb-8 max-w-sm rounded-xl border bg-card p-6 text-left">
            ...
            <ul className="space-y-2 text-sm">
              ...
              <li className="flex items-center gap-2">
                <Check className="h-4 w-4 text-emerald-500" />
                {isFree
                  ? "Switch to 36+ models when needed"
                  : "36+ models included (no extra charge)"}
              </li>
              <li className="flex items-center gap-2">
                <Check className="h-4 w-4 text-emerald-500" />
            ...
            <table className="w-full text-sm">
              <tbody className="divide-y">
                ...
                <tr>
                  ...
                  <td className="px-6 py-4 text-muted-foreground">
                    {model.capabilities.toolCalling ? "Yes" : "No"}
                  </td>
                </tr>
                <tr className="bg-muted/30">
                ...
                <tr>
                  ...
                  <td className="px-6 py-4 text-muted-foreground">
                    {model.capabilities.imageGeneration ? "Yes" : "No"}
                  </td>
                </tr>
                <tr className="bg-muted/30">
                  <td className="px-6 py-4 font-medium">API pricing (input)</td>
                  <td className="px-6 py-4 text-muted-foreground">
                    ${model.pricing.input.toFixed(2)}/1M tokens
                  </td>
                </tr>
                <tr>
                  <td className="px-6 py-4 font-medium">
                  ...
                  <td className="px-6 py-4 text-muted-foreground">
                    ${model.pricing.output.toFixed(2)}/1M tokens
                  </td>
                </tr>
                <tr className="bg-muted/30">
      ...
      {relatedModels.length > 0 && (
        <section className="border-t px-4 py-14">
          <div className="mx-auto max-w-4xl">
            <h2 className="mb-8 text-center font-bold text-2xl">
              Other {providerLabel} models on bedda.ai
            </h2>
      ...
      <section className="border-t bg-muted/30 px-4 py-14">
        <div className="mx-auto max-w-2xl">
          ...
          <div className="space-y-4">
            {faq.map((item) => (
              <div className="rounded-xl border bg-card p-6" key={item.q}>
      ...
      <section className="px-4 py-16 text-center">
        <div className="mx-auto max-w-xl">
          ...
          <Link href="/register">
            <Button className="gap-2" size="lg">
              {isFree ? "Start for free" : "Start free trial"}{" "}
              <ArrowRight className="h-4 w-4" />
            </Button>
          </Link>
        </div>
