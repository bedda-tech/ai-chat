...
export async function POST(request: Request) {
  ...
  const { organizationName, emailDomain, workosOrganizationId, workosConnectionId, idpMetadataUrl } = body

  if (!organizationName || !emailDomain) {
    return Response.json({ error: "organizationName and emailDomain are required" }, { status: 400 })
  }
